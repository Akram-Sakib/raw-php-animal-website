<?php
	...
	if(count($followupmessage)){
		...
		foreach($followupmessage as $followupmsgdata){
			...
			$replyFrom 	= $followupmsgdata['from_name'];
			...
			if($leads){
				...
				foreach($leads as $leaddata){
					...
					$firstname = $lastname = '';	
					$namearray = explode(" ", trim($fromname));
					/* echo '<pre>';
					print_r($namearray); 
					echo '</pre>'; */
					...
					if($followup_count < $ttlFollowupCount)
					{
						...
						{
							echo $chkFollupSent = $objAdmin->checkFollowSentEmails($followupid, $leadid);
							
							if($i>=$followup_count && !$chkFollupSent)
							{
								...
								$mail->ClearReplyTos();
								$mail->addReplyTo($replyto, $replyFrom);
								...
								if($mail->send()){
									...
									echo '<br/>sentId:::><b style="color:red;">'.$replyto.'=>'.$lead.'=>'.$followupid.'</b>';
