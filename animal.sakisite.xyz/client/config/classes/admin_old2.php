...
class Admin{
	...
	public function update_lead_sent_email($leadid, $replyid){
		...
		$query2 = $this->db->prepare("UPDATE `message` SET `hits`=`hits`+1 WHERE `id`=$replyid");
	...
	public function get_followup(){
		$query = $this->db->prepare("SELECT  `followup_messages`.id as followupid , `followup_messages`.* , `mail_settings`.* FROM `followup_messages` LEFT JOIN `mail_settings` ON  `followup_messages`.mail_setting_id=`mail_settings`.id ORDER BY `followup_messages`.`msg_interval`");
	...
	public function leadsovertime($replyinterval, $followupid){
		...
		echo $withinterval;	
		echo "<br/>";
		//echo "SELECT   `emails`.* FROM `emails`  WHERE `time` <= $withinterval AND   `followup` < $followupid ORDER By `id` ASC LIMIT 20";
