...
$submit = mysqli_real_escape_string($db->link, $_GET['submit']);
      ...
      <div class="container">
        <div class="main_section_aftr_row clearfix">
          <div class="row">
		    ...
		    <!-- Pagination -->
            <?php
$query = "SELECT * FROM tbl_post WHERE title LIKE '%$search%' OR body LIKE '%$search%' LIMIT $start_form,$per_page";
$search_post = $db->select($query);
if ($search_post) {
    while ($result = $search_post->fetch_assoc()) {?>
            ...
            <?php }} else {?>
                <div class="my_card">
                    <h2 class="mt-1">Your Search Query Not Found !!.</h2>
                </div>
              <?php }?>

                <?php
$row_query = "SELECT * FROM tbl_post WHERE title LIKE '%$search%' OR body LIKE '%$search%' LIMIT $start_form,$per_page";
...
$pagination_query = "SELECT * FROM tbl_post WHERE title LIKE '%$search%' OR body LIKE '%$search%' ";
    ...
    for ($i = 1; $i <= $total_pages; $i++) {
        echo "<a href='search.php?search=$search&submit=$submit&page=$i'>" . $i . "</a>";
    }
    echo "<a href='search.php?search=$search&submit=$submit&page=$total_pages'>Last Page</a>";
    ?>
                </div>
              </div>
              <?php }?>
